...
def call_amazon_authentication():
    ...
    if response.status_code==200:
        ...
        if access_token:
            ...
            report_type = "GET_RESTOCK_INVENTORY_RECOMMENDATIONS_REPORT" #30days EU
            marketplace_ids = ["A1F83G8C2ARO7P", "A1RKKUPIHCS9HS", "A13V1IB3VIYZZH", "A1PA6795UKMFR9", "APJ6JRA9NG5V4"]
            report_name= "30days_EU.txt"
        
            call_amazon_raports(access_token,report_type,marketplace_ids,report_name)
            
            report_type = "GET_FBA_INVENTORY_PLANNING_DATA" #90 days UK
            marketplace_ids = ["A1F83G8C2ARO7P"]
            report_name= "90days_UK.txt"
            
            call_amazon_raports(access_token,report_type,marketplace_ids,report_name)
            
            report_type = "GET_FBA_INVENTORY_PLANNING_DATA" # 90 days Spain
            marketplace_ids = ["A1RKKUPIHCS9HS"]
            report_name= "90days_ES.txt"
        
            call_amazon_raports(access_token,report_type,marketplace_ids,report_name)
            
            report_type = "GET_FBA_INVENTORY_PLANNING_DATA" #90 days France
            marketplace_ids = ["A13V1IB3VIYZZH"]
            report_name= "90days_FR.txt"
        
            call_amazon_raports(access_token,report_type,marketplace_ids,report_name)
            
            report_type = "GET_FBA_INVENTORY_PLANNING_DATA" #90 days DE
            marketplace_ids = ["A1PA6795UKMFR9"]
            report_name= "90days_DE.txt"
        
            call_amazon_raports(access_token,report_type,marketplace_ids,report_name)
            
            report_type = "GET_FBA_INVENTORY_PLANNING_DATA" #90 days Italy
            marketplace_ids =  ["APJ6JRA9NG5V4"]
            report_name= "90days_IT.txt"
...
def call_amazon_specific_report(reportId, access_token,report_name):
    url = f"https://sellingpartnerapi-eu.amazon.com/reports/2021-06-30/reports/{reportId}"  # Embed reportId into the URL
    headers = {
        "Accept": "application/json",
        "x-amz-access-token": f"{access_token}"
    }
...
def call_amazon_document(reportDocumentId, access_token,report_name):
    url = f"https://sellingpartnerapi-eu.amazon.com/reports/2021-06-30/documents/{reportDocumentId}"  # Embed reportId into the URL
    headers = {
        "Accept": "application/json",
        "x-amz-access-token": f"{access_token}"
    }
